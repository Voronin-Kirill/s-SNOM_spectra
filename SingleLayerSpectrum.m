function Spec = SingleLayerSpectrum(EpsS1, EpsS2, R, L, A, Hmin, Hmin0, EpsT1, EpsT2, phi, const, n, om_min, om_max, Num, FF, flag1, Path_layer, d, flag2, Path_substr, N_acc, M_acc, K)
...
switch flag1
   case 1
        EpsilonL=Path_layer;
        fMat=EpsilonL(:,1);
        Eps_Mat=EpsilonL(:,2)+1i*EpsilonL(:,3);
   case 2
    ...
    case 3
        ...
        fMat=Eps_SiO2(:,1);
        Eps_Mat=Eps_SiO2(:,2)+1i*Eps_SiO2(:,3);
    case 4
        fMat=om;
        Einf=6.6;
        wTO=797;
        wLO=973;
        gam=6;
        Eps_Mat=Einf*(om.^2-wLO^2+1i*gam*om)./(om.^2-wTO^2+1i*gam*om);
end
...
for k = 1:(N+1)
    ...
    for l = 1:K
        I(:,:,k,l) = integral_table_for_shift(Px, xq, wq, m, a, c, H(k), 2*d*l);
    end
end

for i = 1:M
    Eps3=interp1(fSub,real(Eps_Sub),om(i),'spline')+1i*interp1(fSub,imag(Eps_Sub),om(i),'spline');
    Eps2=interp1(fMat,real(Eps_Mat),om(i),'spline')+1i*interp1(fMat,imag(Eps_Mat),om(i),'spline');
    beta_1=(Eps2-Eps1)/(Eps2+Eps1);
    CA=charge(Eps1, Eps2, Eps3, d, K);
    kz3=sqrt(Eps3-Eps1*(sin(phi))^2);
    kz2=sqrt(Eps2-Eps1*(sin(phi))^2);
    kz1=sqrt(Eps1)*cos(phi);
    rp1=(Eps2/kz2-Eps1/kz1)/(Eps2/kz2+Eps1/kz1);
    rp2=(Eps3/kz3-Eps2/kz2)/(Eps3/kz3+Eps2/kz2);
    rp=(rp1+rp2*exp(2i*kz2*d*ko(i)))/(1+rp1*rp2*exp(2i*kz2*d*ko(i)));
    E=E0*(1+const*rp)^(2*FF);                         % Far-field reflection coefficient
...
function C = charge(Eps1, Eps2, Eps3, d, M)

    beta1=(Eps2-Eps1)/(Eps2+Eps1);
    beta2=(Eps3-Eps2)/(Eps3+Eps2);
    di=2*d*linspace(1,M,M);
    
    N = 1000;
    kn = linspace(0, 10/d, N);
    dk = kn(2) - kn(1);
    rn = ref(beta1, beta2, d, kn);
...
function r=ref(beta1, beta2, d, k)
    r=beta1-(beta1+beta2.*exp(-2*k.*d))./(1+beta1.*beta2.*exp(-2*k.*d));
end
