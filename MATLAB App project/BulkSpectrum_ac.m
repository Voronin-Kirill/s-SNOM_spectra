function Spec = BulkSpectrum_ac(EpsS1, EpsS2, R, L, A, Hmin, Hmin0, EpsT1, EpsT2, phi, const, n, om_min, om_max, Num, FF, flag, Path, N_acc, M_acc)
...
pz_sample=zeros(1,N); pzn_sample=zeros(1,M); Phase=zeros(1,M);
...
for q = 1:M
    ...
    for k = 1:(N+1)
        ...
        if (k>1 && k<(N+1))
            pzn_sample(q)=pzn_sample(q)+pz_sample(k)*cos(pi*(k-1)*n/N)/N;
        end
    end
    Phase(q)=angle(pzn_sample(q));
    if (Phase(q)<0)
        Phase(q)=Phase(q)+2*pi;
    end
    Spec(q,1)=om(q);
    Spec(q,2)=abs(pzn_sample(q)/pzn_sub);
    Spec(q,3)=Phase(q)-pi;
    Spec(q,4)=real(Eps2);
    Spec(q,5)=imag(Eps2);
end
